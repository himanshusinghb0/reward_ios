      ...
      if (!force && !background) {
        ...
        if (cachedStats && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            console.log(
              `[Redux] Using cached stats (age: ${Math.round(
                cacheAge / 1000
              )}s)`
            );
            ...
            if (cacheAge > cacheTTL * 0.8) {
              console.log(
                `[Redux] Stats cache is 80% expired, triggering background refresh`
              );
              setTimeout(() => {
          ...
          console.log(
            `[Redux] Stats cache is stale (age: ${Math.round(
              cacheAge / 1000
  ...
  reducers: {
    ...
    setUserFromLogin: (state, action) => {
      ...
      if (user && typeof user === "object" && !isErrorObject) {
        ...
        console.log("🔑 [ProfileSlice] User data from login stored in Redux:", {
          age: state.details?.age,
          ageRange: state.details?.ageRange,
    ...
    builder
      ...
      .addCase(fetchUserProfile.fulfilled, (state, action) => {
        ...
        if (isErrorObject) {
          ...
          console.warn(
            "🔍 [ProfileSlice] Profile fetch returned error object:",
            payload
          );
      ...
      .addCase(fetchProfileStats.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          console.log(
            ...
            }Updated profile stats with fresh data`
          );
        } else {
          console.log(
            `[Redux] Using cached stats (cache age: ${Math.round(
