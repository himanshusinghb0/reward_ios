    ...
    builder
      ...
      .addCase(fetchUserData.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          state.availableGames = (userDataPayload.available || []).filter(game => {
            const title = game.title || game.name || game.details?.name || '';
            return title.toLowerCase().includes('ios');
          });
          state.inProgressGames = (userDataPayload.in_progress || []).filter(game => {
            const title = game.title || game.name || game.details?.name || '';
            return title.toLowerCase().includes('ios');
          });
          state.completedGames = (userDataPayload.completed || []).filter(game => {
            const title = game.title || game.name || game.details?.name || '';
            return title.toLowerCase().includes('ios');
          });
      ...
      .addCase(fetchOffers.fulfilled, (state, action) => {
        // Filter offers to only include those with "iOS" in the title
        const filteredOffers = (action.payload || []).filter(offer => {
          const title = offer.title || offer.name || offer.details?.name || '';
          return title.toLowerCase().includes('ios');
        });
      ...
      .addCase(fetchGamesBySection.fulfilled, (state, action) => {
        ...
        // Filter games to only include those with "iOS" in the title
        const filteredGames = (action.payload.games || []).filter(game => {
          const title = game.title || game.name || game.details?.name || '';
          return title.toLowerCase().includes('ios');
        });
        ...
        if (!fromCache) {
          state.gamesBySection[uiSection] = filteredGames;
      ...
      .addCase(fetchMostPlayedScreenGames.fulfilled, (state, action) => {
        ...
        state.mostPlayedScreenGames = filteredGames;
        console.log(
        ...
        console.log("[Redux] Most Played Screen games:", filteredGames);
      })
