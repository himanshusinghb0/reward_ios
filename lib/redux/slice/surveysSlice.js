      ...
      if (!force && !background) {
        ...
        if (cachedSurveys && cachedSurveys.length > 0 && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            console.log(
              `[Redux] Using cached surveys (age: ${Math.round(
                cacheAge / 1000
              )}s)`
            );
            ...
            if (cacheAge > cacheTTL * 0.8) {
              console.log(
                `[Redux] Surveys cache is 80% expired, triggering background refresh`
              );
              setTimeout(() => {
          ...
          console.log(
            `[Redux] Surveys cache is stale (age: ${Math.round(
              cacheAge / 1000
    ...
    try {
      ...
      if (!force && !background) {
        ...
        if (cachedOffers && cachedOffers.length > 0 && cacheTimestamp) {
          ...
          if (cacheAge < cacheTTL) {
            console.log(
              `[Redux] Using cached non-game offers (age: ${Math.round(
                cacheAge / 1000
              )}s)`
            );
            ...
            if (cacheAge > cacheTTL * 0.8) {
              console.log(
                `[Redux] Non-game offers cache is 80% expired, triggering background refresh`
              );
              setTimeout(() => {
                const { store } = require("@/lib/redux/store");
                store.dispatch(
          ...
          // Cache is stale but exists - return it and refresh in background
          console.log(
            `[Redux] Non-game offers cache is stale (age: ${Math.round(
              cacheAge / 1000
            )}s), refreshing in background`
          );
      ...
      .addCase(fetchSurveys.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          console.log(
            ...
            }Updated surveys with fresh data (${state.surveys.length} surveys)`
          );
        } else {
          console.log(
            `[Redux] Using cached surveys (cache age: ${Math.round(
              (action.payload.cacheAge || 0) / 1000
            )}s, ${action.payload.surveys?.length || 0} surveys)`
          );
        }
      ...
      .addCase(fetchNonGameOffers.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          console.log(
            `[Redux] ${
              isBackground ? "[BACKGROUND] " : ""
            }Updated non-game offers with fresh data (${state.nonGameOffers.length} offers)`
          );
        } else {
          console.log(
            `[Redux] Using cached non-game offers (cache age: ${Math.round(
              (action.payload.cacheAge || 0) / 1000
            )}s, ${action.payload.offers?.length || 0} offers)`
          );
        }
